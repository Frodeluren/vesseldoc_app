...
class Tools {
  ...
  Future<bool> login(String username, String password) async {
    ...
    if (response.statusCode == 200) {
      ...
      token = decData["token"];
      getListOfAvailableFormStructures();
      return true;
    } else {
      return false;
    }
  ...
  Future<bool> getListOfAvailableFormStructures() async {
    listOfAvailStructures = new List<FormStructure>();
  ...
  Future<bool> uploadStructure(String title, dynamic data) async {
    final path = "/structure/set?title=" + title;
      ...
      var request =
          http.MultipartRequest('POST', Uri.parse(url + path));
